...
class TcpSocketConnection {
  ...
  connectWithCommand(
      int timeOut, String separator, String eos, Function callback,
  ...
  ///  * @param  callback  function called when received a message. It must take a 'String' as param which is the message received.
  ///  * @param  attempts  number of attempts before stop trying to connect. Default is 1.
  connect(int timeOut, Function callback, Function connectionLost,
      {int attempts = 1}) async {
    if (_ipAddress == null) {
      print("Class not initialized. You must call the constructor!");
      return;
  ...
  void destroy() {
    if (_server != null) {
      try {
        _server!.destroy();
        _printData("Socket destroy successfully");
      } catch (e) {
        print("ERROR");
      }
    }
